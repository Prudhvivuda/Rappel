...
def find_threshold(scores, threshold):
    ...
    if threshold[0] == 'elbow':  
        ...
        if len(scores) < threshold[1]: 
            # if number is greater than node count, count all as core 
            threshold_score = scores[-len(scores)]
        else:
            # negative index to get threshold score
...
def draw_network_traces(G, pos, node_colors, node_sizes):
    ...
    for node in G.nodes():
        ...
        origin_text = f"Contributed in: {G.nodes[node]['origin']}"
        node_text.append(contributions_text + '<br>' + connections_text + '<br>' + origin_text)
